...
static std::string		gNamePrefix = "vc";		// generated file names start with this prefix, changeable via command line "-n <prefix>" option
			...
			if (mLoading)
			{
				if (mVoiceCommandActive)
				{
					if (mLunaMediaCapture->getReady() && !mCapturing && !mLunaMediaCapture->getAudioCapture())
					{
						...
						mFileName = string_printf("VoiceCommandTest/%s_%02d-%02d_%02dh%02dm%02ds_Vol%d-%d.wav", gNamePrefix.c_str(),
								times.tm_mon + 1, times.tm_mday, times.tm_hour, times.tm_min, times.tm_sec, mMicGain, ++mRecordingCount);
...
int main(int argc, char **argv)
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "gdhsr:n:")) != -1)
	{
		switch (opt)
		{
		case 'g':
...
#define THIS_TEST "vctest"
#define OTHER_TEST "nptest"
